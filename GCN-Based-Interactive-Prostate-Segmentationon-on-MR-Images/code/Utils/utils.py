...
def rgb_img_read(img_path):
    ...
    if img.dtype == 'uint8':
        # [0,1] image
        img = img.astype(np.float32)/255  # normalization tp [0, 1]
...
def overlap_pre_gt(imgpath, polys_pred, polys_gt):
    '''
    :param imgpath: image root path
    :param polys_pred: list [(x, y)]
    :param polys_gt: list [(x, y)]
    :return:
    '''
    #print(imgpath)
    img_ob = Image.open(imgpath)
    img_draw = ImageDraw.Draw(img_ob, 'RGBA')
    ##only draw first_v, last_v
    #img_draw.point(polys_pred[0], fill=(255, 0, 0))
    # img_draw.point(polys_pred[-1], fill=(0, 0, 255))
    ## draw point all by 3 part
    #img_draw.point(polys_pred[:int(len(polys_pred))], fill=(255, 0, 0))
    #img_draw.ellipse(polys_pred,fill=(0,0,255),width=3)
    # img_draw.point(polys_pred[int(len(polys_pred)*0.5):int(len(polys_pred)*0.75)], fill=(0, 255, 0))
    # img_draw.point(polys_pred[int(len(polys_pred) * 0.75):], fill=(0, 0, 255))
    ## draw gt and pre polygon

    # img_draw.polygon(polys_pred, outline='red') #pred polygons
    # img_draw.polygon(polys_gt, outline='blue')  #GT polygons
    #
    # polys_pred = [tuple((item[0] - 1, item[1] - 1)) for item in polys_pred]
    # polys_gt = [tuple((item[0] - 1, item[1] - 1)) for item in polys_gt]
    #
    # img_draw.polygon(polys_pred, outline=(255, 0, 0))  # pred polygons
    # img_draw.polygon(polys_gt, outline=(0, 0, 255))
    #
    # polys_pred = [tuple((item[0] + 1, item[1] + 1)) for item in polys_pred]
    # polys_gt = [tuple((item[0] + 1, item[1] + 1)) for item in polys_gt]
    #
    ...
    # img_draw.polygon(polys_gt, outline=(0, 0, 255))
    # ##top
    # polys_pred = [tuple((item[0], item[1] - 1)) for item in polys_pred]
    # polys_gt = [tuple((item[0], item[1] - 1)) for item in polys_gt]
    #
    # img_draw.polygon(polys_pred, outline=(255, 0, 0))  # pred polygons
    # img_draw.polygon(polys_gt, outline=(0, 0, 255))
    # ##bottom
    ...
    # img_draw.polygon(polys_gt, outline=(0, 0, 255))
    # ## left
    # polys_pred = [tuple((item[0] - 1, item[1])) for item in polys_pred]
    # polys_gt = [tuple((item[0] - 1, item[1])) for item in polys_gt]
    #
    # img_draw.polygon(polys_pred, outline=(255, 0, 0))  # pred polygons
    # img_draw.polygon(polys_gt, outline=(0, 0, 255))
    # ## right
    ...
    # draw circle
    # img_draw = ImageDraw.Draw(img_ob, 'RGBA')
    # img_draw.polygon(polys_pred, outline=(255,193,37))  # pred polygons
    # imsave_ob = np.array(img_ob).astype(np.uint8)
    # for index, item in enumerate(polys_pred):
    #     if index % 32 == 0:
    ...
    # pil_img.save(imgpath[:-56]+''+imgpath[-17:-4]+'_overlay_pre_gt.png', 'PNG')

    img_ob.save('' + imgpath[-17:-4] + '_overlay_pre_gt.png', 'PNG')
